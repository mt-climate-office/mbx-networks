...
NAME_REMAPPER = {
    "timeintointerval": "timeintointervaliftime",
    "iftime": "timeintointervaliftime",
    "int": "intfix",
    "fix": "intfix",
    "sdi12sensorsetup": "sdi12setupresponse",
    "sdi12sensorresponse": "sdi12setupresponse",
    "pulsecount": "pulsecountpulsecountreset",
    "pulsecountreset": "pulsecountpulsecountreset",
    "ln": "logorln",
    "log": "logorln",
    "daylightsaving": "daylightsavingdaylightsavingus",
    "daylightsavingus": "daylightsavingdaylightsavingus",
}
...
def parse_table(table_soup: BeautifulSoup) -> list[ArgOptions]:
    ...
    for row in rows:
        ...
        if len(cols) > 0:
            options.append(
                ArgOptions(cols[0].text.strip().split(" ")[0], cols[1].text.strip())
            )
...
class CSIFunction:
    ...
    def __str__(self):
        ...
        if "*args" in args:
            return_stmt = (
                f'return f"{self.name}({return_args},{{",".join(x for x in args)}})"'
            )
        else:
            return_stmt = f'return f"{self.name}({return_args})"'
        return dedent(f'''
...
def get_meta(description: Tag) -> str:
    meta = {"table": None, "type": None, "description": []}

    while "PopupHeadingTopic" not in (
        description := description.find_next_sibling()
    ).get("class", []):
...
if __name__ == "__main__":
    ...
    for fun in ALL_FUNCTIONS:
        fdef = get_function_def(fun)
        if fdef is not None:
            fun_defs.append(fdef)

    with open("./data.pickle", "wb") as p:
        pickle.dump(fun_defs, p)

    # with open("./data.pickle", "rb") as p:
    #     fun_defs = pickle.load(p)
